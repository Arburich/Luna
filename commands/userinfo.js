...
exports.run = async(client, message, args, level) => {
	...
	let embed = new Discord.RichEmbed()
		...
		.addField("Nickname:", `${(member.nickname !== null && member.nickname !== undefined) ? ` Nickname: $ {
			member.nickname
		}
` : "No nickname"}`, true)
...
` : "Not playing anything."}`, true)
		.addField("Roles", `${member.roles.filter(r => r.id !== message.guild.id).map(roles => ` \ `${roles.name}\``).join(" **|** ") || "No Roles"
}
`, true)
		.addField("Joined", ` $ {
	moment().diff(moment.utc(member.joinedAt), 'days')
}
Days Ago(Created $ {
	moment.utc(member.user.createdAt).format("MMMM Do YYYY")
})`, true)
